...
stdenv.mkDerivation (finalAttrs: {
  ...
  configurePhase =
          ...
          build =
            {
              build = build;
              host = [ host ];
              target = ([ target ] ++ lib.optional (!fastCross) "wasm32-unknown-unknown");
    ...
    ''
      ...
      # maybe try this instead? remarshal  -in json -out toml ${builtins.toJSON rustcConfig} > config.toml
      cp ${formatter.generate "${finalAttrs.pname}-config" rustcConfig} config.toml
      cat config.toml
